...
class CodeGenerator:
    ...
    def __init__(self, system: 'SymbolicDynamicalSystem'):
        ...
        self._f_funcs: Dict[str, Optional[Callable]] = {
            'numpy': None,
            'torch': None,
            'jax': None
        }
        
        self._h_funcs: Dict[str, Optional[Callable]] = {
            'numpy': None,
            'torch': None,
            'jax': None
        }
        ...
        self._A_funcs: Dict[str, Optional[Callable]] = {
            'numpy': None,
            'torch': None,
            'jax': None
        }
        
        self._B_funcs: Dict[str, Optional[Callable]] = {
            'numpy': None,
            'torch': None,
            'jax': None
        }
        
        self._C_funcs: Dict[str, Optional[Callable]] = {
            'numpy': None,
            'torch': None,
            'jax': None
        }
    ...
    def generate_dynamics(self, backend: str, **kwargs) -> Callable:
        ...
        if not callable(func):
            raise RuntimeError(
                f"generate_function returned non-callable object: {type(func)}"
            )
    ...
    def get_jacobians(self, backend: str) -> Tuple[Optional[Callable], Optional[Callable], Optional[Callable]]:
        """
        ...
        """
        return (
            self._A_funcs.get(backend),
            self._B_funcs.get(backend),
            self._C_funcs.get(backend)
        )
    ...
    def __str__(self) -> str:
        ...
        compiled_count = sum(
            1 for backend in ['numpy', 'torch', 'jax']
            if self._f_funcs[backend] is not None
        )
