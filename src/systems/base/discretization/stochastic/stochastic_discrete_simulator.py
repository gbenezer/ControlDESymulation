...
class StochasticDiscreteSimulator(DiscreteSimulator):
    ...
    def simulate_monte_carlo(
        ...
        if parallel and n_paths > 1:
            ...
            return MonteCarloResult(
                states=states, controls=controls, noise=noise_samples, n_paths=n_paths, steps=steps
            )
    ...
    def simulate_antithetic(
        ...
        return MonteCarloResult(
            states=states, controls=controls, noise=None, n_paths=2 * n_pairs, steps=steps
        )
    ...
    def get_info(self) -> dict:
        ...
        base_info["noise_type"] = (
            self.system.get_noise_type().value
            if hasattr(self.system, "get_noise_type")
            else "unknown"
        )
