...
class StochasticDiscretizer:
    ...
    def _cache_constant_diffusion(self):
        ...
        """
        if self._is_additive:
            self._cached_diffusion = self.continuous_system.get_constant_noise(backend=self.backend)
    ...
    def _is_julia_sde_method(self, method: str) -> bool:
        ...
        julia_sde_methods = [
            m
            for m, backend in SDEIntegratorFactory._METHOD_TO_BACKEND.items()
            if backend == "numpy"
        ]
        return method in julia_sde_methods

    def __call__(
        self, x: ArrayLike, u: Optional[ArrayLike] = None, w: Optional[ArrayLike] = None
    ) -> ArrayLike:
        """
    ...
    def _linearize_drift_euler(self, Ac: ArrayLike, Bc: ArrayLike) -> Tuple[ArrayLike, ArrayLike]:
    ...
    def _linearize_drift_exact(self, Ac: ArrayLike, Bc: ArrayLike) -> Tuple[ArrayLike, ArrayLike]:
    ...
    def _linearize_drift_tustin(self, Ac: ArrayLike, Bc: ArrayLike) -> Tuple[ArrayLike, ArrayLike]:
    ...
    def _linearize_diffusion(self, g_eq: ArrayLike, method: str) -> ArrayLike:
        """
...
def create_stochastic_discretizer(
    sde_system: "StochasticDynamicalSystem", dt: float, method: Optional[str] = None, **kwargs
