...
class MonteCarloResult:
    ...
    def get_final_statistics(self) -> Dict[str, ArrayLike]:
        ...
        return {
            key: value[-1] for key, value in all_stats.items()
        }
    ...
    def compute_probability(
        self,
        condition: Callable[[ArrayLike], ArrayLike],
    ...
    def compute_expectation(
        self,
        function: Callable[[ArrayLike], float],
        time_step: Optional[int] = None
    ) -> float:
        """
...
def combine_results(results: list) -> MonteCarloResult:
    ...
    for r in results[1:]:
        if r.steps != steps:
            raise ValueError(
                f"Incompatible steps: {r.steps} vs {steps}"
            )
        if r.states.shape[1:] != first.states.shape[1:]:
            raise ValueError(
                f"Incompatible state shapes: {r.states.shape} vs {first.states.shape}"
            )
    
    # Concatenate all paths
    if isinstance(first.states, np.ndarray):
        all_states = np.concatenate([r.states for r in results], axis=0)
        all_controls = np.concatenate([r.controls for r in results], axis=0) if first.controls is not None else None
        all_noise = np.concatenate([r.noise for r in results], axis=0) if first.noise is not None else None
    
    elif TORCH_AVAILABLE and isinstance(first.states, torch.Tensor):
        import torch
        all_states = torch.cat([r.states for r in results], dim=0)
        all_controls = torch.cat([r.controls for r in results], dim=0) if first.controls is not None else None
        all_noise = torch.cat([r.noise for r in results], dim=0) if first.noise is not None else None
    
    elif JAX_AVAILABLE and isinstance(first.states, jnp.ndarray):
        import jax.numpy as jnp
        all_states = jnp.concatenate([r.states for r in results], axis=0)
        all_controls = jnp.concatenate([r.controls for r in results], axis=0) if first.controls is not None else None
        all_noise = jnp.concatenate([r.noise for r in results], axis=0) if first.noise is not None else None
