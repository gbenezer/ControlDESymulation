...
def create_bivariate_var(
    ...
    ... )
    """
    A = np.array([[persistence1, coupling12], [coupling21, persistence2]])
    ...
    Sigma_w = np.array([[var1, cov], [cov, var2]])
...
def create_macro_var(
    ...
    A = 0.7 * np.eye(n_variables) + 0.1 * (
        np.ones((n_variables, n_variables)) - np.eye(n_variables)
    )
