...
# DIAGNOSTIC: Test integration first
print("="*60)
print("DIAGNOSTIC - Testing SDE integration")
print("="*60)

# Test 1: Simple integration
stochastic_system.set_default_backend('torch')
x0 = np.array([0.5, 0.0])
t_eval_test = np.linspace(0, 10, 1001)

print("\nTest 1: Basic integration with t_eval")
test_result = stochastic_system.integrate(
    x0,
    u=None,
    t_span=(0, 10),
    method='euler',
...
print(f"  t type: {type(test_result['t'])}")
print(f"  t shape: {test_result['t'].shape}")
print(f"  x shape: {test_result['x'].shape}")

# Test 2: Without t_eval
print("\nTest 2: Integration WITHOUT t_eval")
test_result2 = stochastic_system.integrate(
    x0,
    u=None,
    t_span=(0, 10),
    method='euler',
...
print(f"  t shape: {test_result2['t'].shape}")
print(f"  x shape: {test_result2['x'].shape}")

# Convert and check
t_np_test = test_result['t'].cpu().numpy() if hasattr(test_result['t'], 'cpu') else test_result['t']
x_np_test = test_result['x'].cpu().numpy() if hasattr(test_result['x'], 'cpu') else test_result['x']

print(f"\nAfter numpy conversion:")
print(f"  t_np shape: {t_np_test.shape}")
print(f"  x_np shape: {x_np_test.shape}")
print(f"  t range: [{t_np_test.min():.2f}, {t_np_test.max():.2f}]")

if len(t_np_test) < 100:
    print(f"\n❌ ERROR: Only {len(t_np_test)} time points!")
    print("  Integration not working as expected")
    print("  Switching to n_paths parameter instead...")
    
    # Alternative: Use n_paths for Monte Carlo
    print("\nTest 3: Using n_paths parameter for Monte Carlo")
    mc_result = stochastic_system.integrate(
        x0,
        u=None,
        t_span=(0, 10),
        method='euler',
        dt=0.01,
        n_paths=20,  # Monte Carlo directly
        seed=0
    )
    print(f"  Result with n_paths:")
    print(f"    t shape: {mc_result['t'].shape}")
    print(f"    x shape: {mc_result['x'].shape}")
    
    # Convert
    t_mc = mc_result['t'].cpu().numpy() if hasattr(mc_result['t'], 'cpu') else mc_result['t']
    x_mc = mc_result['x'].cpu().numpy() if hasattr(mc_result['x'], 'cpu') else mc_result['x']
    
    print(f"    After conversion:")
    print(f"      t_mc shape: {t_mc.shape}")
    print(f"      x_mc shape: {x_mc.shape}")
    
    if x_mc.ndim == 3 and x_mc.shape[1] > 100:
        print(f"  ✅ SUCCESS with n_paths!")
        use_n_paths = True
    else:
        print(f"  ❌ Still not working")
        use_n_paths = False
else:
    print(f"  ✅ t_eval working: {len(t_np_test)} points")
    use_n_paths = False
...
# Now do actual Monte Carlo based on what works
if use_n_paths:
    # Use n_paths parameter
    ...
    x_batch = result['x'].cpu().numpy() if hasattr(result['x'], 'cpu') else result['x']
    n_trials = x_batch.shape[0]
else:
    # Use loop with individual integrations
    n_trials = 20
...
print(f"Final batch shape: {x_batch.shape} (n_trials={n_trials}, T={len(t_np)}, nx=2)")
...
This demonstrates **true stochastic dynamics** from Wiener process noise:

**What you see:**
- **Individual trajectories**: Each colored line shows one noise realization
- **Stochastic spread**: Trajectories diverge over time from accumulated noise
- **Nonlinear dynamics**: Damping limits growth but uncertainty remains

**Key insight**: Starting from identical initial conditions (θ=0.5, ω=0), the 20 trajectories spread due to different noise realizations. This visualizes inherent stochastic uncertainty.
:::

::: {.callout-tip}
## SDE Integration: Output Time Points

For PyTorch SDE integration, use **`t_eval`** to get dense output:
...
result = system.integrate(
    ...
    t_eval=t_eval,    # Output time points (dense)
    seed=42
)
```

**Why this matters:**
- `dt=0.01` controls integration accuracy (internal time step)
- `t_eval` controls which points are returned in the output
- Without `t_eval`, may only return start/end points
- For plotting, always use `t_eval` for dense, uniform output
:::
...
This example demonstrates **true stochastic dynamics** from the stochastic pendulum's Wiener process noise. Each trajectory experiences different random realizations of the noise process.

**Key features:**
- **Batched input**: `x_batch` shape is `(20, 1001, 2)` = (n_trials, T, nx)
- **Automatic statistics**: Plotter computes mean and std across trials
- **Visual encoding**: Shaded region shows ±1 standard deviation
- **Individual trajectories**: Thin lines show each realization (optional)

For deterministic systems, Monte Carlo typically varies initial conditions or parameters rather than having inherent stochasticity.
:::

::: {.callout-tip}
## Backend Conversion for Plotting

The plotter requires **NumPy arrays**, but SDE integration used **PyTorch**:
```python
...
- PyTorch SDE solvers work with torch tensors
- Plotly requires NumPy arrays for visualization
- The plotter handles this automatically for single trajectories, but for batched data we do it explicitly
:::
