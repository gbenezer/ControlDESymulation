...
print(f"✓ Pendulum created: {pendulum.nx} states, {pendulum.nu} controls")

# Evaluate at a point
x = np.array([np.pi + 0.1, 0.0])
u = np.array([0.0])
dx = pendulum(x, u)
print(f"✓ Dynamics evaluated: dx/dt = {dx}")

# Linearize
A, B = pendulum.linearize(np.array([np.pi, 0.0]), np.array([0.0]))
print(f"✓ Linearization computed: A shape = {A.shape}")
```
````
...
print(f"State: {x}")
print(f"Control: {u}")
print(f"Types are semantic, not just structural! ✓")
```
...
print(f"✓ Integration successful: {result['nfev']} function evaluations")
```
...
print(f"  ✓ Verified!" if np.allclose(riccati_residual, 0, atol=1e-10) else "✗ Failed")
```
...
try:
    ...
    print(f"✓ Converted to PyTorch: {type(x_torch)}")
except ImportError:
...
u = np.array([0.0])
result = f_numpy(x, u)
print(f"✓ Function evaluates: f({x}) = {result}")
```
...
print(f"✓ Simulation completed: {result['nsteps']} steps")
```
