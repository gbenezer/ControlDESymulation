...
Tests cover:
- LQR design (continuous and discrete)
...
class ControlTestCase(unittest.TestCase):
    ...
    def assert_stable_continuous(self, eigenvalues: np.ndarray):
        ...
        self.assertLess(
            max_real, 0,
            f"System unstable. Max Re(λ) = {max_real}"
        )
    ...
    def assert_stable_discrete(self, eigenvalues: np.ndarray):
        ...
        self.assertLess(
            max_mag, 1.0,
            f"System unstable. Max |λ| = {max_mag}"
        )


# ============================================================================
# LQR Continuous Tests
# ============================================================================
...
class TestLQRContinuous(ControlTestCase):
    ...
    def test_lqr_high_q_vs_r(self):
        ...
        Q_high = 100 * self.Q2
        result_high_q = design_lqr_continuous(
            self.A_double_int,
            self.B_double_int,
            Q_high,
            self.R1
        ...
        R_high = 10 * self.R1
        result_high_r = design_lqr_continuous(
            self.A_double_int,
            self.B_double_int,
            self.Q2,
            R_high
...
class TestLQRDiscrete(ControlTestCase):
    ...
    def test_lqr_discrete_vs_continuous_consistency(self):
        ...
        result_c = design_lqr_continuous(
            self.A_double_int,
            self.B_double_int,
            self.Q2,
            self.R1
...
class TestKalmanFilter(ControlTestCase):
    ...
    def test_kalman_perfect_measurement(self):
        """Test with very low measurement noise (R â†' 0)."""
        R_low = 1e-6 * np.eye(1)
...
class TestLQG(ControlTestCase):
    ...
    def test_lqg_separation_principle(self):
        ...
        lqr_result = design_lqr_discrete(
            self.Ad_double_int,
            self.Bd_double_int,
            self.Q2,
            self.R1
...
class TestControllability(ControlTestCase):
    ...
    def test_controllability_full_rank(self):
        ...
        result = analyze_controllability(
            self.A_double_int,
            self.B_double_int
        )
...
class TestErrorHandling(ControlTestCase):
    ...
    def test_lqr_dimension_mismatch(self):
        ...
        with self.assertRaises(ValueError):
            design_lqr_continuous(A, B_wrong, self.Q2, self.R1)
...
class TestIntegration(ControlTestCase):
    ...
    def test_lqr_stabilizes_unstable_system(self):
        ...
        Q = np.eye(2)
        R = np.array([[1.0]])
        result = design_lqr_continuous(A_unstable, B, Q, R)
        
        # Closed-loop should be stable
        stability_cl = analyze_stability(
            A_unstable - B @ result['gain'],
            system_type='continuous'
        )
    ...
    def test_full_lqg_pipeline(self):
        ...
        ctrl = analyze_controllability(
            self.Ad_double_int,
            self.Bd_double_int
        )
        self.assertTrue(ctrl['is_controllable'])
        
        obs = analyze_observability(
            self.Ad_double_int,
            self.C_double_int
        )
