...
from src.systems.base.numerical_integration.stochastic.sde_integrator_base import ConvergenceType
...
class TestInitialization:
    ...
    def test_initialization_with_method(self, ou_system, dt):
        ...
        for method in ["EM", "SRIW1"]:  # ← Fixed: Use Julia method names
            discretizer = StochasticDiscretizer(ou_system, dt=dt, method=method, backend="numpy")
    ...
    def test_initialization_with_backend(self, ou_system, dt):
        ...
        for backend, method in backends_and_methods:
            discretizer = StochasticDiscretizer(ou_system, dt=dt, backend=backend, method=method)
    ...
    def test_initialization_with_custom_integrator(self, ou_system, dt, seed):
        ...
        discretizer = StochasticDiscretizer(ou_system, dt=dt, integrator=custom_integrator)
...
class TestStepFunction:
    ...
    def test_step_reproducibility_with_noise_jax(self, ou_system, dt):
        ...
        discretizer = StochasticDiscretizer(ou_system, dt=dt, method="Euler", backend="jax")
    ...
    @pytest.mark.skipif(not _jax_available(), reason="JAX required for reproducibility test")
    def test_step_with_seed_reproducibility_jax(self, dt, seed):
        ...
        assert_allclose(jnp.asarray(x_next1), jnp.asarray(x_next2), rtol=1e-6)

    @pytest.mark.skipif(not _torch_available(), reason="PyTorch required for batched test")
...
class TestNoiseHandling:
    ...
    def test_custom_noise_diagnostic(self):
        ...
        discretizer = StochasticDiscretizer(ou_system, dt=0.01, method="euler", backend="torch")
    ...
    @pytest.mark.skipif(
        not _jax_available(), reason="JAX required - only backend supporting custom noise"
    )
...
class TestStatisticalProperties:
    ...
    def test_brownian_mean_zero(self, brownian_system, dt):
        ...
        # 99% CI: ±2.576*SE ≈ ±0.08
        assert abs(empirical_mean) < 0.1, f"Mean {empirical_mean:.4f} should be near 0"
    ...
    def test_brownian_variance_scales_with_time(self, brownian_system):
        ...
        # Allow 30% tolerance
        assert 1.4 < ratio < 2.6, f"Variance ratio {ratio:.2f} should be near 2.0"
    ...
    def test_ou_process_mean_reversion(self, ou_system, dt):
        ...
        assert (
            0.5 * expected_mean < empirical_mean < 1.5 * expected_mean
...
class TestBackendSupport:
    ...
    def test_torch_backend(self, ou_system, dt, seed):
        ...
        discretizer = StochasticDiscretizer(ou_system, dt=dt, backend="torch", seed=seed)
...
class TestDeterministicComparison:
    ...
    @pytest.mark.skipif(
        not _jax_available(), reason="JAX required for zero-noise deterministic comparison"
    )
    def test_zero_noise_matches_deterministic_jax(self, dt):
        ...
        stoch_disc = StochasticDiscretizer(ou_system, dt=dt, method="Euler", backend="jax")
        det_disc = Discretizer(det_system, dt=dt, method="euler", backend="jax")
