...
from src.systems.base.discretization.stochastic.stochastic_discrete_simulator import (
    StochasticDiscreteSimulator,
)
...
class TestSingleTrajectory:
    ...
    def test_return_noise(self):
        ...
        states, noise = sim.simulate(
            x0, steps=10, controller=lambda x, k: np.array([0.0]), return_noise=True
        )
    ...
    def test_return_controls_and_noise(self):
        ...
        states, controls, noise = sim.simulate(
            x0, steps=10, controller=controller, return_controls=True, return_noise=True
        )
...
class TestMonteCarlo:
    ...
    def test_monte_carlo_statistics(self):
        ...
        result = sim.simulate_monte_carlo(
            x0, steps=50, n_paths=500, controller=lambda x, k: np.array([0.0])
        )
    ...
    def test_monte_carlo_with_controller(self):
        ...
        x0 = np.array([2.0])
        result = sim.simulate_monte_carlo(x0, steps=30, n_paths=200, controller=controller)
    ...
    def test_monte_carlo_return_controls(self):
        ...
        controller = lambda x, k: (
            np.array([0.1 * k]) if x.ndim == 1 else np.full((x.shape[0], 1), 0.1 * k)
        )

        x0 = np.array([0.0])
        result = sim.simulate_monte_carlo(
            x0, steps=10, n_paths=50, controller=controller, return_controls=True
        )
    ...
    def test_monte_carlo_return_noise(self):
        ...
        x0 = np.array([0.0])
        result = sim.simulate_monte_carlo(x0, steps=10, n_paths=50, return_noise=True)
    ...
    def test_monte_carlo_parallel_vs_sequential(self):
        ...
        # Parallel (default)
        result_parallel = sim.simulate_monte_carlo(x0, steps=20, n_paths=100, parallel=True)

        # Sequential
        sim.set_seed(42)  # Reset seed
        result_sequential = sim.simulate_monte_carlo(x0, steps=20, n_paths=100, parallel=False)
...
class TestAntitheticVariates:
    ...
    def test_antithetic_variance_reduction(self):
        ...
        result_std = sim_std.simulate_monte_carlo(x0, steps=30, n_paths=200, controller=controller)
        stats_std = result_std.get_statistics()

        # Antithetic variates
        sim_anti = StochasticDiscreteSimulator(system, seed=42)
        result_anti = sim_anti.simulate_antithetic(x0, steps=30, n_pairs=100, controller=controller)
...
class TestNoiseHandling:
    ...
    def test_custom_noise_sequence(self):
        ...
        states = sim.simulate(x0, steps=5, controller=lambda x, k: np.array([0.0]), noise=noise)
...
class TestStatisticalAnalysis:
    ...
    def test_mean_convergence_monte_carlo(self):
        ...
        result = sim.simulate_monte_carlo(
            x0, steps=5, n_paths=1000, controller=lambda x, k: np.array([0.0])
        )
...
class TestDeterministicComparison:
    ...
    def test_zero_noise_matches_deterministic_mean(self):
        ...
        states = sim.simulate(
            x0, steps=10, controller=lambda x, k: np.array([0.0]), noise=noise_zero
        )
    ...
    def test_monte_carlo_mean_approaches_deterministic(self):
        ...
        result = sim.simulate_monte_carlo(x0, steps=10, n_paths=5000, controller=controller)
...
class TestEdgeCases:
    ...
    def test_return_final_only(self):
        ...
        # Full trajectory
        states_full = sim.simulate(x0, steps=100, controller=lambda x, k: np.array([0.0]))

        # Final only
        sim.set_seed(42)
        x_final = sim.simulate(
            x0, steps=100, controller=lambda x, k: np.array([0.0]), return_final_only=True
        )
...
class TestNumericalAccuracy:
    ...
    def test_ar1_stationary_variance(self):
        ...
        x0 = np.array([0.0])
        result = sim.simulate_monte_carlo(
            x0, steps=200, n_paths=1000, controller=lambda x, k: np.array([0.0])
        )
...
class TestPerformance:
    ...
    def test_large_batch_simulation(self):
        ...
        result = sim.simulate_monte_carlo(
            ...
            n_paths=1000,
            controller=lambda x, k: (
                torch.zeros(x.shape[0], 1) if x.ndim > 1 else torch.tensor([0.0])
            ),
        )
