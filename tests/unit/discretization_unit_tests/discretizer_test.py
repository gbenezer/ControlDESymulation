...
from src.systems.base.discretization.discretizer import DiscretizationMethod, Discretizer
...
# Import test systems
from src.systems.builtin.linear_systems import AutonomousLinearSystem, LinearSystem, LinearSystem2D
...
def linear_system():
    ...
    system.add_equilibrium("origin", x_eq=np.array([0.0]), u_eq=np.array([0.0]), verify=True)
    return system
...
def pendulum_system():
    ...
    system = SymbolicPendulum(m_val=1.0, l_val=0.5, beta_val=0.1, g_val=9.81)

    # Add equilibria after creation
    system.add_equilibrium("downward", x_eq=np.array([0.0, 0.0]), u_eq=np.array([0.0]), verify=True)

    system.add_equilibrium(
        "upright", x_eq=np.array([np.pi, 0.0]), u_eq=np.array([0.0]), verify=True
    )
...
class TestInitialization:
    ...
    def test_initialization_with_custom_integrator(self, linear_system, dt):
        ...
        discretizer = Discretizer(linear_system, dt=dt, integrator=custom_integrator)
...
class TestLinearization:
    ...
    def test_linearize_nonlinear_system(self, pendulum_system, dt):
        ...
        assert np.all(np.abs(eigenvalues) < 1.0), (
            f"Downward equilibrium should be stable. " f"Eigenvalues: {eigenvalues}"
        )
    ...
    def test_linearize_at_unstable_equilibrium(self, pendulum_system, dt):
        ...
        assert np.any(np.abs(eigenvalues) > 1.0), (
            f"Upright equilibrium should be unstable. " f"Eigenvalues: {eigenvalues}"
        )
    ...
    def test_linearize_at_nonequilibrium(self, linear_system, dt):
        ...
        Ad_eq, Bd_eq = discretizer.linearize(np.array([0.0]), np.array([0.0]), method="euler")
    ...
    def test_invalid_linearization_method(self, linear_system, dt):
        ...
        with pytest.raises(ValueError, match="Unknown linearization method"):
            discretizer.linearize(np.array([0.0]), np.array([0.0]), method="invalid_method")
...
class TestBackendSupport:
    ...
    def test_batched_evaluation_torch(self, linear_system, dt):
        ...
        discretizer = Discretizer(linear_system, dt=dt, backend="torch", method="euler")
        ...
        for i in range(3):
            ...
            assert_allclose(x_next_batch[i].numpy(), x_next_i.numpy(), rtol=1e-6)

    def test_julia_batching_warning_behavior(self, linear_system, dt):
        """Test that Julia backend issues warning for batched input."""
        discretizer = Discretizer(linear_system, dt=dt, method="Tsit5", backend="numpy")
